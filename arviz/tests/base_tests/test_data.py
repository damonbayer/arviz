...
class TestInplaceOverloads:
    """Test that @overload decorators work correctly for inplace parameter.
    
    These tests verify that methods with inplace parameter return the correct
    types at runtime, which should match their type annotations.
    """

    def test_sel_inplace_returns_none(self):
        """Test that sel with inplace=True returns None."""
        idata = load_arviz_data("centered_eight")
        result = idata.sel(chain=[0], inplace=True)
        assert result is None

    def test_sel_inplace_false_returns_idata(self):
        """Test that sel with inplace=False returns InferenceData."""
        idata = load_arviz_data("centered_eight")
        result = idata.sel(chain=[0], inplace=False)
        assert isinstance(result, InferenceData)
        assert result is not idata

    def test_isel_inplace_returns_none(self):
        """Test that isel with inplace=True returns None."""
        idata = load_arviz_data("centered_eight")
        result = idata.isel(chain=[0], inplace=True)
        assert result is None

    def test_isel_inplace_false_returns_idata(self):
        """Test that isel with inplace=False returns InferenceData."""
        idata = load_arviz_data("centered_eight")
        result = idata.isel(chain=[0], inplace=False)
        assert isinstance(result, InferenceData)
        assert result is not idata

    def test_stack_inplace_returns_none(self):
        """Test that stack with inplace=True returns None."""
        idata = load_arviz_data("centered_eight")
        result = idata.stack(sample=["chain", "draw"], inplace=True)
        assert result is None

    def test_stack_inplace_false_returns_idata(self):
        """Test that stack with inplace=False returns InferenceData."""
        idata = load_arviz_data("centered_eight")
        result = idata.stack(sample=["chain", "draw"], inplace=False)
        assert isinstance(result, InferenceData)
        assert result is not idata

    def test_unstack_inplace_returns_none(self):
        """Test that unstack with inplace=True returns None."""
        idata = load_arviz_data("centered_eight")
        idata.stack(sample=["chain", "draw"], inplace=True)
        result = idata.unstack(inplace=True)
        assert result is None

    def test_unstack_inplace_false_returns_idata(self):
        """Test that unstack with inplace=False returns InferenceData."""
        idata = load_arviz_data("centered_eight")
        idata.stack(sample=["chain", "draw"], inplace=True)
        result = idata.unstack(inplace=False)
        assert isinstance(result, InferenceData)
        assert result is not idata

    def test_rename_inplace_returns_none(self):
        """Test that rename with inplace=True returns None."""
        idata = load_arviz_data("centered_eight")
        result = idata.rename({"theta": "theta_new"}, inplace=True)
        assert result is None

    def test_rename_inplace_false_returns_idata(self):
        """Test that rename with inplace=False returns InferenceData."""
        idata = load_arviz_data("centered_eight")
        result = idata.rename({"theta": "theta_new"}, inplace=False)
        assert isinstance(result, InferenceData)
        assert result is not idata

    def test_rename_vars_inplace_returns_none(self):
        """Test that rename_vars with inplace=True returns None."""
        idata = load_arviz_data("centered_eight")
        result = idata.rename_vars({"theta": "theta_new"}, inplace=True)
        assert result is None

    def test_rename_vars_inplace_false_returns_idata(self):
        """Test that rename_vars with inplace=False returns InferenceData."""
        idata = load_arviz_data("centered_eight")
        result = idata.rename_vars({"theta": "theta_new"}, inplace=False)
        assert isinstance(result, InferenceData)
        assert result is not idata

    def test_rename_dims_inplace_returns_none(self):
        """Test that rename_dims with inplace=True returns None."""
        idata = load_arviz_data("centered_eight")
        result = idata.rename_dims({"school": "school_new"}, inplace=True)
        assert result is None

    def test_rename_dims_inplace_false_returns_idata(self):
        """Test that rename_dims with inplace=False returns InferenceData."""
        idata = load_arviz_data("centered_eight")
        result = idata.rename_dims({"school": "school_new"}, inplace=False)
        assert isinstance(result, InferenceData)
        assert result is not idata

    def test_map_inplace_returns_none(self):
        """Test that map with inplace=True returns None."""
        idata = load_arviz_data("centered_eight")
        result = idata.map(lambda x: x, groups="posterior", inplace=True)
        assert result is None

    def test_map_inplace_false_returns_idata(self):
        """Test that map with inplace=False returns InferenceData."""
        idata = load_arviz_data("centered_eight")
        result = idata.map(lambda x: x, groups="posterior", inplace=False)
        assert isinstance(result, InferenceData)
        assert result is not idata

    def test_concat_inplace_returns_none(self):
        """Test that concat with inplace=True returns None."""
        idata1 = load_arviz_data("centered_eight")
        idata2 = load_arviz_data("centered_eight")
        result = concat(idata1, idata2, dim="chain", inplace=True)
        assert result is None

    def test_concat_inplace_false_returns_idata(self):
        """Test that concat with inplace=False returns InferenceData."""
        idata1 = load_arviz_data("centered_eight")
        idata2 = load_arviz_data("centered_eight")
        result = concat(idata1, idata2, dim="chain", inplace=False)
        assert isinstance(result, InferenceData)
        assert result is not idata1
        assert result is not idata2
